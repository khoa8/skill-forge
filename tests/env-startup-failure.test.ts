...
describe("SKILLFORGE_ENV_FILE isolation for production harnesses", () => {
  it("a hostile cwd .env cannot defeat the explicit env file (server uses the file, discovery skipped)", async () => {
    // cwd .env would ABORT startup if it were read (invalid provider);
    // the explicit file pins mock. Server must come up healthy on mock.
    const hostile = await makeFixture("SKILLFORGE_PROVIDER=definitely-not-a-provider\n");
    const explicitDir = await mkdtemp(join(tmpdir(), "skillforge-env-isolation-"));
    ...
    // PORT rides in the explicit file: binding the expected ephemeral port
    // proves the child actually consumed THIS file (and not discovery).
    await writeFile(explicitPath, `SKILLFORGE_PROVIDER=mock\nPORT=${port}\n`, "utf8");
    try {

      const child = spawn(process.execPath, [join(repoRoot, "dist", "server", "index.js")], {
        cwd: hostile.dir, // hostile .env lives exactly where discovery would look
        env: {
          ...process.env,
          SKILLFORGE_ENV_FILE: explicitPath,
          SKILLFORGE_DATA_ROOT: join(explicitDir, ".data-store"),
          SKILLFORGE_ACKNOWLEDGE_EXPOSURE: "1",
          // Do not inherit the developer's provider pin — the isolation
          // contract under test is cwd-.env vs explicit env file.
          ...Object.fromEntries(Object.entries(process.env).filter(([k]) => k !== "SKILLFORGE_PROVIDER")),
        },
        stdio: ["ignore", "pipe", "pipe"],
      });
      let stdout = "";
      ...
      child.stdout!.on("data", (c: Buffer) => (stdout += c.toString()));
      child.stderr!.on("data", (c: Buffer) => (stderr += c.toString()));
      // Teardown is unconditional — no orphaned server even on assertion failure.
      ...
      try {
        expect(healthy, `server did not come up healthy: ${stderr.slice(0, 300)}`).toBe(true);
        expect(stdout).toContain("listening");
      } finally {
        stop();
        await Promise.race([stopped, new Promise<void>((r) => setTimeout(r, 2_000))]);
      }
    } finally {
  ...
  it("a missing explicit env file fails cleanly without listening", async () => {
    const missing = join(tmpdir(), "skillforge-env-isolation-missing.env");
