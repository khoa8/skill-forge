...
**State:** P0 complete and verified; P1 shipped; P1.5 (production readiness) shipped; **remediation run in progress** on `feature/production-readiness-remediation` — audit findings fixed (canonical metadata check, source-note propagation, public-only GitHub policy, actual-byte bounds, ingestion deadline, dependency advisories).
...
- **P1 shipped:**
  ...
  - **16 deterministic validators** (was 14): added `eval-integrity` and `provenance-integrity`. Now **17** with `canonical-metadata-consistency` (remediation).
...
- `npm test` → **179/179 passing** (166 at the production-readiness branch head; +5 canonical-metadata, +3 source-notes, +1 public-only policy, +2 actual-byte bound, +2 ingestion deadline during remediation).
...
| `npm test` | Vitest suite (179 tests) |
