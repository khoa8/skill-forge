...
**URL safety limits:** http/https only; refused hosts are refused by name, and every address — literal or DNS-resolved, including each redirect target — must classify as **globally reachable** under an allowlist check (private, loopback, link-local, unique-local, multicast, documentation/test/benchmark ranges, and embedded-IPv4 tunneling forms are all rejected; malformed input fails closed); connections are pinned to the validated records at connection time (no DNS re-resolution/rebinding gap); max 3 redirects; 1 MB cap; one 15 s end-to-end deadline covering DNS, redirects, connection, and body streaming (a stalled response fails with `url_deadline_exceeded`, HTTP 504, instead of hanging); only text-like content types; page JavaScript is never executed (JS-rendered pages are reported as empty rather than guessed at).
