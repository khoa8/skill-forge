...
- **Codebase mode**: analyzes the repository as a software project for coding agents. SkillForge fetches the repo metadata and one recursive tree, filters it through a codebase-specific allowlist (source/config/test/instruction files in; binaries, generated/minified output, lockfile contents, and build/vendor directories out), then deterministically ranks and selects a **bounded, prioritized, diversity-capped** set of files for deep analysis — 60 files / 200 KB per file / 1.4 MB total / depth 10; per-request timeout 15 s; overall deadline 90 s; tree listing capped at 10 MB. Selection is computed locally (never by the model). The generated skill covers stack, manifests, evidenced commands (from `package.json` scripts, CI workflows, and build configs), source/test layout, entrypoints, repository conventions (from `AGENTS.md`/`CONTRIBUTING.md`-style files), and tests — every claim carries the inspected file it came from, and the analysis states exactly how many files were inspected vs. skipped.
