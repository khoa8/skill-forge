...
| --- | --- | --- | --- |
| `claude-code` | Claude Code | `<skill-id>/SKILL.md` with `name` + `description` YAML front matter, plus canonical supporting files (`references/`, `workflows/`, `examples/`, `evals/`, `manifest.json`) | Anthropic Agent Skills format: `name` ≤64 chars strict kebab-case (lowercase letters/digits, single internal hyphens), no XML tags, none of the reserved words `anthropic`/`claude`; `description` non-empty ≤1024 chars, no XML tags; optional `compatibility` ≤500 chars when present. The exporter fails closed on the same rule set that target-aware validation enforces, using semantic YAML parsing to preserve other front-matter keys (license, allowed-tools, metadata, compatibility, Claude Code extension fields) while keeping the slug name synchronized. Structure verified by this repo's tests. |
...
The contract basis is the Anthropic Agent Skills documentation plus the published `anthropics/skills` skill-creator validator: `name` must be 1–64 chars of strict kebab-case (no leading/trailing or consecutive hyphens), contain no XML tags, and contain neither reserved word `anthropic` nor `claude` (case-insensitive substring); `description` must be non-empty, at most 1024 chars, with no angle brackets (the validator’s proxy for “no XML tags”); the optional standard field `compatibility`, when present and non-empty, must be a string of at most 500 chars (mirroring the validator, which treats falsy values as absent). Target-aware deterministic validation fails these cases, and `exportClaudeCode()` throws `export_claude_metadata_invalid` (HTTP 422) instead of emitting a violating package. No closed allowlist of front-matter keys is enforced — Claude Code extension fields are preserved untouched when the exporter resynchronizes the slug name, and valid packages pass post-transformation validation. This is an authoring-format check, not a claim about runtime behavior inside Claude Code.
