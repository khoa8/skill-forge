...
export function qualifyEditedFileContent(path: string, content: string, sourceName: string): string {
  if (path === "SKILL.md") {
    ...
    const generatorBannerRe = /^> Generated by SkillForge(?:\s+\([^)]+\))?\s+from\s+[`"']?[^`"'\n]+[`"']?\.\s+Every factual claim below is grounded in that source;\s+explicit gaps are marked\./m;
  ...
  if (path.startsWith("references/")) {
    let result = content;
    const refHeaderRe = /^> Excerpt from source "[^"]*" \(lines \d+[-–]\d+\)\. Verbatim except for this header; relative links to the original repository are shown as paths instead of links\./m;
    ...
    // Remove exact generator-owned footer if present at the end of the file
    const refFooterRe = /\n\n_Source: [^,\n]+, lines \d+[-–]\d+\._\n?$/;
  ...
  if (path.startsWith("workflows/")) {
    let result = content;
    const wfHeaderRe = /^> Documented procedure from source "[^"]*" \(lines \d+[-–]\d+\)\. Steps are verbatim from the source; relative links are shown as paths\./m;
    if (wfHeaderRe.test(result)) {
      result = result.replace(
        wfHeaderRe,
  ...
  if (path.startsWith("examples/")) {
    const exampleHeaderRe = /^((\/\/|#)\s*Source:\s*[^,\n]+,\s*lines\s*\d+[-–]\d+\s*(?:\([^)\n]*\))?\.\s*Verbatim code block\.?\r?\n)/;
    if (exampleHeaderRe.test(content)) {
      return content.replace(
        exampleHeaderRe,
        `$2 Originally generated from source: ${sourceName}; edited after generation.\n`,
      );
    }
    return content;
  }
