...
| Runtime config | `src/config/env.ts` | `.env` loading (explicit `SKILLFORGE_ENV_FILE` override loads only that file — used by harnesses for isolation; otherwise working dir, then entry-module walk-up; process environment keeps precedence; values never logged) and startup validation: provider id allowlist, non-mock provider requires `SKILLFORGE_API_KEY`, `PORT` range — all enforced before traffic is served. |
...
| IP policy | `src/core/sources/ip-policy.ts` | The single authority for "may URL ingestion connect to this address?" — an ordered CIDR rule table (static reviewed snapshot of the IANA IPv4/IPv6 Special-Purpose Address Registries) evaluated with longest-prefix match, so more-specific global allocations inside special parents (e.g. 192.0.0.9/32) and non-routable reservations inside global parents (e.g. 3fff::/20) are both classified correctly. Embedded-IPv4 forms (mapped, NAT64, 6to4, Teredo) re-classify through the IPv4 engine. Fail closed on unrecognized/malformed input. Pure functions, no I/O. |
